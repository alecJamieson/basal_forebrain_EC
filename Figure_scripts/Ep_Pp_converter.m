...
% Load the .mat file
load('BMA_search_PEB_between_group_analysis_147_participants.mat');
...
for i = 1:length(indices)
    row = ceil(indices(i) / region_n);
    col = mod(indices(i) - 1, region_n) + 1;
    Ep_matrix2(col, row) = BMA.Ep(i + connection_n);  % Doubled for Ep2
    Pp_matrix2(col, row) = BMA.Pp(i + connection_n);  % Doubled for Pp2
end
